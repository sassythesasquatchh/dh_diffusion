...
class GaussianDiffusion:
    ...
    def masked_l2(self, a, b, mask=None):
        ...
        if mask is None:
            mask = torch.ones((a.shape[0],1,1,a.shape[3])).to(a.device)
    ...
    def training_losses(self, model, x_start, t, model_kwargs=None, noise=None, dataset=None):
        ...
        # In our implementation, model_kwargs is the conditioning and is None for the unconstrained case
        ...
        x_t[:, :, :, 0] = x_start[:, :, :, 0].clone()
        x_t[:, :, :, -1] = x_start[:, :, :, -1].clone()
        ...
        if self.loss_type == LossType.KL or self.loss_type == LossType.RESCALED_KL:
            ...
            if self.lambda_rcxyz > 0.:
                ...
                terms["rcxyz_mse"] = self.masked_l2(target_xyz, model_output_xyz)  # mean_flat((target_xyz - model_output_xyz) ** 2)

            if self.lambda_vel_rcxyz > 0.:
                if self.data_rep == 'rot6d' and dataset.dataname in ['humanact12', 'uestc', "GRAB"]:
                    ...
                    terms["vel_xyz_mse"] = self.masked_l2(target_xyz_vel, model_output_xyz_vel)
